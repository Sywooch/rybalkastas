...
use common\traits\UserMongoTrait;
...
class User extends ActiveRecord implements IdentityInterface
{
    ...
    public function rules()
    {
        return [
            ...
            'passwordLength'   => ['password', 'string', 'min' => 3, 'max' => 72, 'on' => ['register', 'create']],
        ];
    }
    ...
    public function resendPassword()
    {
        ...
        $mail = \Yii::$app->mailer->compose(['html'=>'@frontend/views/mail/new_password'],['user'=>$this, 'password'=>$this->password])
            ->setFrom(['support@rybalkashop.ru' => 'Rybalkashop.ru Рыболов на "Птичке"']);
        $mail->setTo($this->email);
        $mail->setSubject("Ваш новый пароль");
        if($mail->send()){
            echo 'Сообщение '.$this->email.' отправлено'.PHP_EOL;
        }
    }
    ...
    public function afterSave($insert, $changedAttributes)
    {
        ...
        if ($insert) {
            ...
            if ($this->_customer == null) {
                $this->_customer = \Yii::createObject(SCCustomers::className());
            }
            $this->_customer->link('user', $this);

            $uploader = new UtUploader;
            if(empty($this->_customer->{'1c_id'})){
                $uploader->UploadCustomer($this->_customer);
            }
        }
    }
    ...
    public function linkOnAir()
    {
        $findCustomer = SCCustomers::find()->where(['email'=>$this->email])->andWhere("Login <> ''")->andWhere("cust_password <> ''")->one();
        if(!empty($findCustomer)){
            $findCustomer->user_id = $this->id;
            $findCustomer->save();
            return true;
        } else {
            return false;
    ...
    public function linkNew()
    {
        ...
        $customer->first_name = $this->profile->name;
    ...
    {
        //return SCCustomers::find()->where(['user_id'=>$this->id])->one();
        return $this->hasOne(SCCustomers::className(), ['user_id'=>'id']);
    }
    ...
    public function getShowNameCustomer()
    {
        if(isset($this->customer) && !empty($this->customer->first_name)){
    ...
    {
        $this->viewProductF($this->id, $category_id);
    }

    public function addProductWaiting($product_id){
        $this->addToWaiting($this->id, $product_id);
    }
    ...
    public function sendTest()
    {
        ...
        //$mailer->
        $mail = $mailer->compose(['html'=>'@frontend/views/mail/new_password'],['user'=>$this, 'password'=>$this->password])
            ->setFrom(['support@rybalkashop.ru' => 'Rybalkashop.ru Рыболов на "Птичке"']);
        $mail->setTo($this->email);
        $mail->setSubject("Ваш новый пароль");
        if($mail->send()){
    ...
    {
        $ids = \Yii::$app->authManager->getUserIdsByRole($role_name);
