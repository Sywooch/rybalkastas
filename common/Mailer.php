...
{
    /** @var string */
    public $viewPath = '@frontend/views/mail';
    ...
    public function getWelcomeSubject()
    {
        if ($this->welcomeSubject == null) {
            $this->setWelcomeSubject("Добро пожаловать");
        }
     ...
     * @return bool
     */
    public function sendWelcomeMessage($user, Token $token = null, $showPassword = false)
    {
     ...
     * @return bool
     */
    public function sendConfirmationMessage($user, Token $token)
    {
        return $this->sendMessage(
            $user->email,
     ...
     * @return bool
     */
    public function sendRecoveryMessage($user, Token $token)
    {
        try{
            return $this->sendMessage(
                $user->email,
