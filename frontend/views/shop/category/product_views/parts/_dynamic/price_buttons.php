...
if (!empty($_GET['product'])) {
    ...
    $model = \common\models\SCProducts::find()
...
<?php if ($model->canAdd <> 1): ?>
    <?php if (Yii::$app->user->isGuest): ?>
        <div class="row">
            <div class="col-md-12">
                <button class="btn btn-block btn-warning text-uppercase btn-flat" type="submit" data-toggle="modal" data-target="#subscribeModal">
...
<div class="wrapper row flex product_actions">
    ...
    <div class="col-md-6 col-sm-12">
        ...
        <?php if ($model->canAdd == 1): ?>
            ...
            <?php if ($model->onlyRetail): ?>
                <div style="font-size: 12px; color: #e93232;">
                    Данный товар можно купить только в магазинах нашей <a href="/page/shops" target="_blank">розничной сети</a>
                </div>
                <br>
            <?php else: ?>
                <button class="btn btn-primary btn-flat btn-block text-uppercase buybtn" type="submit">
        ...
        <?php else: ?>
            <button class="btn btn-block btn-default text-uppercase disabled btn-flat" type="submit">
                <i class="fa fa-times" aria-hidden="true"></i>
                <?= $model->canAdd ?>
            </button>
        <?php endif; ?>

        <?= $this->render("//shop/category/product_views/parts/_brand_button", [
            'model' => $model
        ]) ?>
    </div>
</div>
