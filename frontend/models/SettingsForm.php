...
class SettingsForm extends Model
{
    ...
    protected function defaultEmailChange()
    {
        ...
        try{
            ...
            //$mailer->
            $mail = $mailer->compose(['html'=>'@frontend/views/mail/reconfirmation'],["user"=>$this->user, "token"=>$token])
                ->setFrom(['contacts@rybalkashop.ru' => 'Rybalkashop.ru Рыболов на "Птичке"']);
            $mail->setTo($this->email);
            $mail->setSubject("Изменение адреса электронной почты");
        } catch (\Exception $e){
            \Yii::$app->bot->sendMessage(-14068578, "Пользователю $this->email не отправлено письмо при смене Email!");
        }
