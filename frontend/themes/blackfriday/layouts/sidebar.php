...
<div class="news-sidebar hidden-xs">
    ...
    <?php foreach(\common\models\SCNewsTable::find()->where(['published'=>1])->orderBy('published_at DESC')->limit(5)->all() as $n):?>
    <div class="item-card">
        <?= Html::img(Yii::$app->imageman->load('/news_pictures/'.$n->image, '205x205', 60, false, 'sidebarnewss'), ['alt'=>$n->title_ru])?>
        <div class="date"><?=Yii::$app->formatter->asRelativeTime($n->published_at);?></div>
        <h2><?=$n->title_ru?></h2>
