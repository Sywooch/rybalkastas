...
if(!empty($monufacturer)){
    ...
    if(!empty($page) && !empty($page->link_image)):?>
            <a class="brandicon" href="<?=\yii\helpers\Url::to(['brands/index', 'alias'=>$page->alias])?>"><?=\yii\helpers\Html::img(Yii::$app->imageman->load('/brand_pictures/'.$page->link_image, '120x40', 80, false, 'brand'));?></a>
    <?php endif;
}
