...
<table class="table">
    <tr>
        <td><strong>Код контрагента:</strong></td>
        <td><?= !empty($user->customer)?$user->customer->{'1c_id'}:"Не выгружен" ?></td>
    </tr>
    <tr>
        <td><strong>Клубная карта:</strong></td>
        <td><?= !empty($user->customer->card)?$user->customer->card->number:'<span class="text-danger">Не привязана</span>' ?></td>
    </tr>
    <?php if(empty($user->customer->card)):?>
        <tr>
            <td><strong>Привязать карту:</strong></td>
            <td>
                <?php $form = \yii\widgets\ActiveForm::begin();?>
                    <?=$form->field($model,'card')?>
                    <?=$form->field($model,'user_id')->hiddenInput(['value'=>$user->id])->label(false)?>
                    <button type="submit" class="btn btn-flat btn-primary btn-block">Привязать</button>
