...
$cache = Yii::$app->cache;
$modelM = $cache->getOrSet('sidecats_filter', function() use($model){
...
$optQuery = $cache->getOrSet($key, function () use ($model) {
    $prdAr = \yii\helpers\ArrayHelper::getColumn($model->products, 'productID');
    return \common\models\SCProductOptionsValues::find()
...
?>
<?php
$subcats = $cache->getOrSet(['category_subcats', $model->categoryID], function () use ($model) {
...
<?php $form = \yii\widgets\ActiveForm::begin([
    ...
    <div class="dropdown catalog-dd <?php if(!empty($_GET['SCCategoriesSearch']))echo 'open'?>">
        <button class="btn btn-primary btn-lg btn-flat btn-block dropdown-toggle text-uppercase" type="button"
                id="dropdownMenu1"
        ...
        <ul class="dropdown-menu" id="filterdropdownmenu" aria-labelledby="dropdownMenu1">
                        ...
                        <div class="input-group slider-with-input">
                            <?= \yii\helpers\Html::activeTextInput($searchModel, 'price',
                                [
                                    ...
                                    'data-slider-step' => 5,
                                    'data-slider-value' => "[$searchModel->price]"]); ?>                </div>
            ...
            <?php foreach ($optQuery as $opt): ?>
                <?php if($opt->option->filterType == 'finder' && !Yii::$app->user->can('superField'))continue;?>
